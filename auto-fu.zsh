...
afu-recursive-edit-and-accept () {
  ...
  { zle "${__accepted[@]}"} || {
    ((afu_in_p==1)) && BUFFER="$buffer_cur"; zle accept-line }
}
...
afu-register-zle-accept-line () {
  ...
  $afufun () {
    ...
    # [[ $BUFFER[-1] == " " ]] && [[ $buffer_cur[-1] != ' ' ]] && buffer_cur+=' '
    # [[ $BUFFER[-1] != " " ]] && [[ $buffer_cur[-1] == ' ' ]] && buffer_cur=$buffer_cur[1,-2]
...
auto-fu () {
  ...
  if [[ $words[-1] =~ ".*(\*|\?|<|\||\[|#|\^).*" ]]; then
    ...
    zle complete-word
    [[ $_lastcomp[completer] == "history" ]] && zle complete-word
  fi
...
afu+complete-word () {
  ...
  if ((afu_in_p == 1)); then
    ...
    case $LBUFFER[-1] in
      ...
      (/) # path-ish  ⇒ propagate auto-fu if it could be
        { # XXX: directly modify LBUFFER for preventing the auto-stuff, then
          ...
          local c=$LBUFFER[-1]; LBUFFER=$LBUFFER[1,-2]; zle -U "$c"; return
        };;
    ...
    [[ $lastcompp == t ]] && (( $_lastcomp[nmatches] > 1 )) \
      && [[ $LBUFFER[-1] != [[:space:]] ]] &&
...
auto-fu-zcompile () {
  ...
  ! [[ -x $HOME/.local/bin/zsh-count-files ]] && \
